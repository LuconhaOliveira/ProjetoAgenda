...
FOR EACH ROW
BEGIN
	INSERT INTO tblogcategorias(usuario,idcategoria,dtAcao,acao) VALUES(user(),NEW.tbcategorias.id,now(), concat('a categoria ', new.categoria ,' foi inserida'));
END;
//
...
FOR EACH ROW
BEGIN
	INSERT INTO tblog(usuario,idcategoria,dtAcao,acao) VALUES(user(),OLD.id,now(), concat('a categoria ', old.categoria ,' foi modificada para ', new.categoria));
END;
//
...
FOR EACH ROW
BEGIN
	INSERT INTO tblogcontatos(usuario,idcontato,dtAcao,acao) VALUES(user(),NEW.tbcontatos.id,now(), concat('o contato ', new.nome ,' foi inserido'));
END;
//
...
FOR EACH ROW
BEGIN
	INSERT INTO tblog(usuario,idcontato,dtAcao,acao) VALUES(user(),OLD.id,now(), concat('o contato ', old.nome ,' foi modificada para ', new.nome));
END;
//
